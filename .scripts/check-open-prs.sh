...
# Resolve owner/name even when `gh` cannot map the SSH remote to a host.
...
if [ -z "${REPO}" ] || [[ "${REPO}" != */* ]]; then
  echo "ERROR: could not resolve GitHub repository (set GH_REPO=owner/name)." >&2
  exit 1
fi

PRS_JSON="$(gh pr list -R "${REPO}" --state open --limit 100 --json number,title,labels,body,url,mergeable,mergeStateStatus)"
